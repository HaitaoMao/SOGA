...
def init_layer(args, params, conf_params):
    ...
    # active_func_list = ["relu"] * (args.num_layers - 1) + ["softmax"]
...
def init_log(args):
    with open("./record/" + args.model_name+f"/{args.domain_name}/{args.random_seed}.log", 'w') as f:
        f.truncate()

    logger = logging.getLogger(__name__)
    logger.setLevel(logging.INFO)
    fh = logging.FileHandler("./record/" + args.model_name+f"/{args.domain_name}/{args.random_seed}.log")
...
def save_model(args, prefix, model):
    torch.save({'model_state_dict': model.state_dict()}, f"record/{args.model_name}/{args.domain_name}/{prefix}_{args.random_seed}.pkl")

def load_model(args, prefix, model):
    state_dict = torch.load(f"record/{args.model_name}/{args.domain_name}/{prefix}_{args.random_seed}.pkl", map_location=args.device)["model_state_dict"]
...
def evaluate(output, labels, single, metric):
    if single:
        ...
        for i in range(num_labels):
            macro_score += f1_score(labels[:, i].type(torch.int64),
                                  (preds[:, i] > 0.5).type(torch.int64),
                                  average='macro')
            micro_score += f1_score(labels[:, i].type(torch.int64),
                                  (preds[:, i] > 0.5).type(torch.int64),
                                  average='micro')
        macro_score /= num_labels
        micro_score /= num_labels
...
def test(model, args, data, criterion, mode = 'valid'):
    ...
    acc = evaluate(outputs, labels, args.single, args.metric)
