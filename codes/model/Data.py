...
class DomainData(InMemoryDataset):
    ...
    def process(self):
        ...
        training_size = int(len(random_node_indices) * (1 - self.valid_ratio))
        train_node_indices = random_node_indices[:training_size]
        val_node_indices = random_node_indices[training_size:]

        train_masks = torch.zeros([y.shape[0]], dtype=torch.uint8)
        train_masks[train_node_indices] = True
        val_masks = torch.zeros([y.shape[0]], dtype=torch.uint8)
        val_masks[val_node_indices] = True
